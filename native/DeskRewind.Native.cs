...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        if (args.Length == 0) return Fail("缺少命令。");
        ...
        try
        {
            if (command == "list") { ListProfiles(store, args.Any(a => String.Equals(a, "--json", StringComparison.OrdinalIgnoreCase))); return 0; }
            if (command == "save") { Save(store, name); return 0; }
            if (command == "overwrite") { OverwriteProfile(store, profileId); return 0; }
            if (command == "restore") { Restore(store, profileId, force, GetOption(args, "--expected")); return 0; }
            if (command == "rename") { RenameProfile(store, profileId, name); return 0; }
            if (command == "delete") { DeleteProfile(store, profileId); return 0; }
    ...
    private static void Save(string store, string name)
    {
        ...
        ProfileDocument data = ProfileStore.Load(store);
    ...
    private static void OverwriteProfile(string store, string profileId)
    {
    ...
    private static void Restore(string store, string profileId, bool force, string expected)
    {
        var data = ProfileStore.Load(store);
        ...
        if (selected.shellState == null || selected.shellState.version != 1)
            throw new InvalidOperationException("布局数据无效：缺少图标大小记录。");
        if (!LayoutEnvironment.HasSystemScale(selected))
            throw new InvalidOperationException("布局数据无效：缺少系统缩放记录。");
        IntPtr list;
    ...
    private static void ListProfiles(string store, bool json)
    {
        EnvironmentSnapshot current = LayoutEnvironment.Capture(Desktop.FindListView());
        List<LayoutProfile> profiles = ProfileStore.Load(store).profiles.OrderByDescending(p => LayoutEnvironment.HardwareMatches(p, current)).ThenByDescending(p => p.savedAt).ToList();
        if (json)
        {
            Console.WriteLine(new JavaScriptSerializer().Serialize(profiles.Select(p => new { p.id, p.name, p.savedAt, iconCount = p.icons == null ? 0 : p.icons.Count, environmentToken = LayoutEnvironment.Fingerprint(current), hardwareMatch = LayoutEnvironment.HardwareMatches(p, current), systemScaleMatch = LayoutEnvironment.HasSystemScale(p) && DisplayScale.SameScale(p.monitors, current.monitors), iconGridMatch = LayoutEnvironment.HasIconGrid(p) && LayoutEnvironment.SameIconState(p.shellState, current.shellState) }).ToList()));
            return;
        }
    ...
    private static void RenameProfile(string store, string profileId, string name)
    {
    ...
    private static void DeleteProfile(string store, string profileId)
    {
...
internal static class ProfileStore
{
    public static ProfileDocument Load(string path)
    {
        if (File.Exists(path))
        {
            try
            {
                ProfileDocument loaded = new JavaScriptSerializer().Deserialize<ProfileDocument>(File.ReadAllText(path, Encoding.UTF8));
                if (loaded == null) throw new InvalidOperationException("布局存档为空。");
                if (loaded.profiles == null) loaded.profiles = new List<LayoutProfile>();
                return loaded;
            }
            catch (Exception ex) { throw new InvalidOperationException("布局存档无法读取：" + ex.Message); }
        }
        var created = new ProfileDocument();
        Save(path, created);
        return created;
    }
    ...
    public static void Save(string path, ProfileDocument data)
    {
        ...
        data.format = "DeskRewind.Profiles/v1";
...
internal static class LayoutEnvironment
{
    ...
    public static EnvironmentMismatch Compare(LayoutProfile profile, EnvironmentSnapshot current)
    {
        ...
        return new EnvironmentMismatch { Hardware = !report.hardwareMatch, SystemScale = !report.systemScaleMatch, IconGrid = !report.iconGridMatch };
    }
    ...
    public static EnvironmentReport Report(LayoutProfile profile, EnvironmentSnapshot current)
    {
        ...
        bool scaleMatch = HasSystemScale(profile) && DisplayScale.SameScale(profile.monitors, current.monitors);
        bool gridMatch = HasIconGrid(profile) && SameIconState(profile.shellState, current.shellState);
        return new EnvironmentReport { hardwareMatch = hardware, systemScaleMatch = scaleMatch, iconGridMatch = gridMatch };
    }

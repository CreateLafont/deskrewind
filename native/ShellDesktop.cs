...
public sealed class ShellDesktop : IDisposable
{
    ...
    private void ApplyPersistentGrid(ShellLayoutState saved)
    {
        ...
        // If base metrics are absent, derive them from measured Shell results
        // without shifting icon coordinates.
    ...
    {
        // Preserve Shell flags while restoring the control's pre-operation state.
        var list = FindWindowEx(viewWindow, IntPtr.Zero, "SysListView32", null);
        IntPtr style;
    ...
    {
        if (saved == null || saved.version != 1 || saved.items == null || saved.items.Count == 0) throw new InvalidOperationException("布局数据无效：缺少桌面状态记录。");
        var before = Capture();
